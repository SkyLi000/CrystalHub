...
Inviter.Prompt = function(options)
	...
	if __PROMPT__ == 2 then
		print("[Receiver] - No Prompt Accepted")
		shared.DiscordPrompt = true
	else
		local success, inviteData = pcall(function()
		...
		connections.ignoreClick = Prompt.Frame.Ignore.Activated:Connect(function()
			writefile("GSCConfig/DiscordConfig.GSC", "2")
			disconnect(Prompt)
			toggle(Prompt, false)
			shared.DiscordPrompt = true
		end)
